...
// console.log(accountId + "  heheheehehe  " + streamName);
// console.log('Millicast Viewer Stream: ', streamName);
function organiser_by_user_id(user_id) {
  $.ajax({
    url: '/organiser_id/' + user_id,
    ...
    success: function (data) {
      console.log(data);
      console.log(data.data.unity_url);
      $('#broadcat_video_2').attr('src', data.data.unity_url);
      streamName = data.data.stream_name_2;
      accountId = data.data.account_id;
      console.log('Millicast Viewer Stream: ', streamName);

      //back button
      var anchortag = '/agenda/'+data.data.guest_id;
      $("#backtoagenda").attr('href',anchortag)


    }
  })
};
organiser_by_user_id(params.get('user_id'))
...
function connect() {
  ...
  pc.ontrack = function (event) {
    ...
    //Play it
    let vidWin = document.getElementById('broadcat_video');
    console.log(vidWin);
    // let vidWin = document.getElementsByTagName('video')[0];
    // let vidWin_2 = document.getElementById('broadcat_video_2');
    if (vidWin) {
      vidWin.srcObject = event.streams[0];
      console.log(event.streams[0]);
      vidWin.controls = true;
      // vidWin_2.srcObject = event.streams[0];
      // vidWin_2.controls = true;
    }
  };
...
function ready() {
  // let v = document.getElementsByTagName('video')[0];
    let v = document.getElementById('broadcat_video');
    // let v2 = document.getElementById('broadcat_video_2');
  if (v) {
    v.addEventListener("click", evt => {
      v.play();
    });
    // v2.addEventListener("click", evt => {
    //   v2.play();
    // });
  }
  //connect();
